...
async def grouped_query(
    ...
    """
    Execute a group-aware RAG query with contextual document retrieval and LLM generation.

    This endpoint:
    - Retrieves custom RAG configuration for the group (prompt, tone, temperature)
    - Fetches relevant documents from a group-isolated vectorstore
    - Applies custom prompt engineering logic based on group configuration
    - Executes the query with an LLM
    - Logs query metadata and audit trail into the database
    - Checks/sets a local cache for repeated queries

    Args:
        query (str): The user’s input question.
        user_id (str): The ID of the requesting user.
        group_id (str): The ID of the group context for RAG filtering.
        role (str, optional): Role context for the prompt, defaults to "domain expert".
        db (AsyncSession): Database session dependency.

    Returns:
        dict: Answer generated by the model, along with a `cached` flag indicating cache usage.
    """
